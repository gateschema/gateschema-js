...
export type ValidationResultCacheItem = [Constraint[], any];
...
export interface Keyword {
  ...
  validator: (
    value: any,
    ctx: ValidationCtx | ValidationCtxHighOrder,
    cb: (err?: any) => any
  ) => void;
  getChildSchema?: (
    options: { args: any; childKey: string },
    cb: (schema?: GateSchemaBase) => any
  ) => any;
  getMatchSchema?: (
    options: { path: string; args: any; rootData: any; state: ValidationState },
    cb: (schema?: GateSchemaBase) => any
  ) => any;
}
...
export type ValidateBreak = symbol;

export type ValidateResult = undefined | ValidationError | ValidateBreak;

export type ValidationCallback = (err?: null | ValidationError | Error) => any;
...
export interface GateSchemaBaseConstructArgs {
  0: undefined | { constraints?: Constraint[] };
}
export interface GateSchemaBaseConstructor {
  ...
  formatMsg(
    msg: string,
    detail: ValidationErrorMsgParams,
    constraint: Constraint
  ): string;
  ...
  getValue(
    ...
    data: any,
    currentPath: string,
    values: SchemaValidationCtx['state']['values']
  ): any;
  getSchema(
    path: string,
    options: {
...
export interface GateSchemaBase {
  ...
  defaultOptions: ValidationOptions;
  initConstraints: Constraint[]; // prototype property
  ...
  validate(
    value: any,
    options?: ValidationOptions
  ): Promise<null | ValidationError | Error>;
  toJSON(): Constraint[];
  [key: string]: any;
}
...
export interface GateSchemaKeywords {
  ...
  any: GateSchema;
  enum(definition: { [key: string]: number }): GateSchema;
  enumList(definition: { [key: string]: number }): GateSchema;
  list(schema: GateSchema): GateSchema;
  map(definition: { [key: string]: GateSchema }): GateSchema;
  oneOf(schemas: GateSchema[]): GateSchema;
  value(value: any): GateSchema;

  switch(
    path: string,
    cases: Array<{ case: GateSchema; schema: GateSchema }>
  ): GateSchema;
...
export type GetSchemaCallback = (_: GateSchema) => any;
...
export interface GateSchemaConstructor
  extends GateSchemaKeywords,
